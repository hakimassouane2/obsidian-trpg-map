...
| | |
|---|---|
| Dossier source | `code/jdr/obsidian-plugins/obsidian-trpg-map/` |
| Branche de travail | `staging` |
| Identifiant du plugin | `obsidian-trpg-maps` |
| Cible de déploiement | `<vault>/.obsidian/plugins/obsidian-trpg-maps/` |
...
```powershell
..\..\obsidian-plugin-tools\deploy-plugin.ps1 obsidian-trpg-map     # Windows
```

```bash
../../obsidian-plugin-tools/deploy-plugin.sh obsidian-trpg-map      # macOS
```
...
- **Ne jamais éditer les fichiers dans `.obsidian/plugins/obsidian-trpg-maps/`.** C'est une cible
